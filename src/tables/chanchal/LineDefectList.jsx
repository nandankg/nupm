...
const LineDefectList = () => {
  ...
  const { toPDF, targetRef } = usePDF({ filename: "LineDefectList.pdf" });
  const [fromDate, setFromDate] = useState(dayjs().startOf("day"));
  const [toDate, setToDate] = useState(dayjs().endOf("day"));
  const [searchValue, setSearchValue] = useState("");
  const dispatch = useDispatch();
  const assetregisters = useSelector((state) => state.lineDefect);
  ...
  return (
    <div className="container mt-5">
      ...
      <PDFExportComponent
        ...
        filename="LineDefectList.pdf"
      />
      <ExcelExportComponent
        data={currentItems}
        columns={linedefect}
        fileName="LineDefectList.xlsx"
      />
      {/* Pagination Controls */}
