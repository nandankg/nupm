...
import {
  ...
  fetchData,
  saveData,
} from "../../reducer/chanchal/EquFaiRegReducer";
...
const EquFaiRegList = () => {
  ...
  const { toPDF, targetRef } = usePDF({ filename: "EquFaiRegList.pdf" });
  const [fromDate, setFromDate] = useState(dayjs().startOf("day"));
  const [toDate, setToDate] = useState(dayjs().endOf("day"));
  const [searchValue, setSearchValue] = useState("");
  const dispatch = useDispatch();
  const assetregisters = useSelector((state) => state.equFaiReg);
 ...
 useEffect(() => {
  ...
  return (
    <div className="container mt-5">
      <h1> Equipment Failure Station Register</h1>
      <ReusableFilterBar
      ...
      />
      <TableStructure
        columns={equipmentfailureregister}
      ...
      <PDFExportComponent
        ...
        filename="EquFaiRegList.pdf"
      />
      <ExcelExportComponent
        data={currentItems}
        columns={equipmentfailureregister}
        fileName="EquFaiRegList.xlsx"
      />
      {/* Pagination Controls */}
