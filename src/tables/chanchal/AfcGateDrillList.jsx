...
import { occafcgatedrill } from "../../data/tableColumns";
...
console.log(deprt);
const AfcGateDrillList = () => {
  const tableRef = useRef(null);
  const { toPDF, targetRef } = usePDF({ filename: "AfcGateDrillList.pdf" });
  const [fromDate, setFromDate] = useState(dayjs().startOf("day"));
  const [toDate, setToDate] = useState(dayjs().endOf("day"));
  const [searchValue, setSearchValue] = useState("");
  const dispatch = useDispatch();
  const assetregisters = useSelector((state) => state.afcGateDrill);
  ...
  return (
    <div className="container mt-5">
      ...
      <PDFExportComponent
        ...
        filename="AfcGateDrillList.pdf"
      />
      <ExcelExportComponent
        data={currentItems}
        columns={occafcgatedrill}
        fileName="AfcGateDrillList.xlsx"
      />
      {/* Pagination Controls */}
...
export default AfcGateDrillList;
