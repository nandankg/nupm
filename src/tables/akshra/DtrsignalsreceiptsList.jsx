...
const DtrsignalsreceiptsList = () => {
  const tableRef = useRef(null);
  const { toPDF, targetRef } = usePDF({ filename: "dtr_receipt_signals.pdf" });
  ...
  useEffect(() => {
    if (DTrsignalsreceiptsList.data && DTrsignalsreceiptsList.data.data) {
      setItems(DTrsignalsreceiptsList.data.data);
      setSlug(DTrsignalsreceiptsList.slug);
      setFilteredItems(DTrsignalsreceiptsList.data.data);
    }
  ...
  const handleFilter = () => {
    const newData = items.filter((row) => {
      ...
      const received_name = row.received_name
        ? row.received_name.toLowerCase()
        : "";
      const ledger_no = row.ledger_no ? row.ledger_no.toLowerCase() : "";
      ...
      return (
        (challan_no.includes(searchValue.toLowerCase()) ||
          received_name.includes(searchValue.toLowerCase()) ||
          ledger_no.includes(searchValue.toLowerCase())) &&
        isInDateRange
      );
    });
    ...
    <div className="container">
      ...
      <div className="d-flex justify-content-between">
        ...
        <div className="d-flex gap-3">
          <Link to=""></Link>
          <DownloadTableExcel
            filename="Dtr_receipt_table"
            sheet="Dtr_receipt_table"
        ...
        >
          <thead>
            ...
            <tr>
              ...
              <th rowSpan={1}>Challan NO.& Date</th>
              <th rowSpan={1}>Challan Date</th>
              <th rowSpan={1}>Name</th>
              <th rowSpan={1}>Desig</th>
              {/* <th rowSpan={1}>Sign</th>*/}
              <th rowSpan={1}>For What Work & Location</th>
              {/*<th rowSpan={1}>Sign of Receiver</th>*/}
          ...
          <tbody>
            {filteredItems.map((item, index) => (
              <tr key={index}>
                ...
                <td>{item.received_designation}</td>
                {/*<td>{item.sign}</td>*/}
...
export default DtrsignalsreceiptsList;
