...
const UpsRoomEntryRegisterForm = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const [slug, setSlug] = useState(getLastParameter().trim());
  const [date, setDate] = useState(new Date());
  const [s_no, setSno] = useState(1);
  const upsRoomEntry = useSelector((state) => state.data);
  ...
  const validateForm = () => {
    ...
    // Business rule validations - UPS Room specific
    ...
    // UPS Room safety validation
    if (formData.purpose && !formData.purpose.toLowerCase().includes("maintenance") && !formData.purpose.toLowerCase().includes("inspection") && !formData.purpose.toLowerCase().includes("authorized")) {
  ...
  const breadcrumbItems = [
    { text: "UPS Room Entry Register", to: "#" },
    { text: "Register", to: "#" }
  ];

  return (
    <OperationFormLayout
      title="UPS Room Entry Register"
      formType="Operation"
    ...
    >
      ...
      <div className='row mb-3'>
        <div className='col-md-3'>
          <UniversalOperationFormField
            ...
            placeholder="e.g., UPS Maintenance, Inspection"
          />
        </div>
      ...
      {/* UPS Room Safety Notice */}
      <div className="row mb-3">
        <div className="col-md-12">
          <div className="alert alert-warning" role="alert">
            <strong>UPS Room Safety:</strong> Authorized personnel only. Ensure proper safety protocols are followed when entering UPS room facilities.
          </div>
        </div>
      </div>
...
export default UpsRoomEntryRegisterForm;