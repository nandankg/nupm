...
import { editData,fetchData } from "../../reducer/store/HalfYearlyMaintenanceFormReducer";
...
const EditHalfYearlyMaintenance = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const location = useLocation();
  const [items, setItems] = useState([]);
    const { id } = location.state;
  const mjl11List = useSelector((state) => state.pmdepothalfyearly);
...
export default EditHalfYearlyMaintenance;
