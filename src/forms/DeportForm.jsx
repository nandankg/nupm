...
import { addData, adddeportform } from "../reducer/DepotyearlyReducer";
          ...
          >
            <form onSubmit={handleSubmit}>
              <div className=" mb-3 form-heading-container">
                <h3 className="form-heading">Deport Form Yearly </h3>
          ...
          <div className="d-flex gap-3 justify-content-between">
            {["BR1", "BR2"].map((m) => (
              <div key={m} className="col-md-4">
                <label className="form-label" htmlFor={`${workKeys}${m}${indexs}`}>{m}</label>
                <input
                  type="text"
                  className="form-control"
                  id={`${workKeys}${m}${indexs}`}
                  name={`${workKeys}${m}${indexs}`}
                  value={formValues[workKeys][indexs][m] || ""}
                  onChange={(e) =>
                    handleInputChanges(workKeys, indexs, m, e.target.value)
                  }
                  placeholder={m}
                />
              </div>
